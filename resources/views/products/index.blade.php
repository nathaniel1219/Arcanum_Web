...
@section('content')
            ...
            @foreach ($products as $index => $product)
                <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition overflow-hidden">
                    <img src="{{ asset('images/products/' . $product->image_url) }}" alt="{{ $product->product_name }}"
                        class="w-full h-48 object-cover cursor-pointer" onclick="openModal({{ $index }})">
    ...
    {{-- Optional Toast --}}
    <div id="toast"
        class="fixed bottom-6 right-6 bg-green-500 text-white py-3 px-4 rounded shadow-lg hidden z-50 transition-opacity">
    </div>
@endsection
